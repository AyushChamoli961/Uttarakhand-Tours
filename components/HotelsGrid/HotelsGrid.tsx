...
const HotelsGrid = ({ hotels }: any) => {
    ...
    const handlePrevClick = () => {
        setCurrentDestinationIndex((prevIndex) =>
            prevIndex === 0 ? hotels.length - cardsToShow : prevIndex - 1
        );
    };

    const handleNextClick = () => {
        setCurrentDestinationIndex((prevIndex) =>
            prevIndex === hotels.length - cardsToShow ? 0 : prevIndex + 1
        );
    };

    const isPrevDisabled = currentDestinationIndex === 0;
    const isNextDisabled = currentDestinationIndex >= hotels.length - 1;
    if (!hotels.length) {
        return (
